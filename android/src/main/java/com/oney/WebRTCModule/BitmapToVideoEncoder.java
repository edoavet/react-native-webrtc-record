...
public class BitmapToVideoEncoder {
    ...
    public void startEncoding(int width, int height, File outputFile, double frameRate) {
    ...
    private void encodeYUV420SP(byte[] yuv420sp, int[] argb, int width, int height) {
        ...
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i++) {
                ...
                if (j % 2 == 0 && index % 2 == 0) {
                    yuv420sp[frameSize + uvIndex] = (byte) ((U < 0) ? 0 : ((U > 255) ? 255 : U));
                    yuv420sp[frameSize + uvCount + uvIndex] = (byte) ((V < 0) ? 0 : ((V > 255) ? 255 : V));
                    uvIndex++;
                }
    ...
    private long computePresentationTime(long frameIndex, double framerate) {
        return (long)(132 + frameIndex * 1000000 / framerate);
    }
}
